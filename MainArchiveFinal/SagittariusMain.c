...
#define COUNTS_PER_REV     7950.0f //265.0f   // your measured value (you already use 265)
...
{
  // --- 0) Reset heading PID and center steering ---
  Motor_forward_reset();
  Steering_ToUS(0);
  HAL_Delay(50);

  // --- 1) Snapshot starting encoder count ---
  int16_t start_pos = position;
  float g_travelled_cm = 0.0f;

  // --- 2) Parameters ---
  const float slow_down_cm = 5.0f; // start slowing down when within 25 cm
  const float creep_cm     =  1.0f; // creep in the last 8 cm
  const float tol_cm       =  1.5f; // tolerance band in cm
  ...
  while (1) {
    ...
    if (diff < 0) diff = -diff; // absolute counts

    g_travelled_cm = counts_to_cm(diff);
    float remaining = target_cm - g_travelled_cm;

    // Stop if within tolerance band
    if (remaining <= tol_cm) {
      Motor_stop();
      break;
    }
    ...
    // --- PWM scheduling ---
    int pwm = base_pwm;
    if (remaining < slow_down_cm) pwm = (int)(0.6f * base_pwm); // slow down
    if (remaining < creep_cm)     pwm = pwmMin + 60;            // creep mode

    // --- Drive forward with heading PID ---
    Motor_forward(pwm);

    HAL_Delay(10); // loop ~100 Hz
  }
  ...
  float err_pct = (g_travelled_cm - target_cm) / target_cm * 100.0f;
  sprintf(buf, "Target=%.1fcm, Travel=%.1fcm, Err=%.1f%%\r\n",
          target_cm, g_travelled_cm, err_pct);
...
int main(void)
{
  ...
  run_straight_to_distance_cm(80.0,5000);
