...
Note missionImpossibleTheme[] = {
    // First phrase: E4 E4 F4 G4
    {NOTE_E4, 150}, {NOTE_E4, 150}, {NOTE_F4, 150}, {NOTE_G4, 150},
    // Second phrase: E4 E4 F4 G4 (repeat of the first)
    {NOTE_E4, 150}, {NOTE_E4, 150}, {NOTE_F4, 150}, {NOTE_G4, 150},
    // Third phrase: B4 C5 B4 C5
    {NOTE_B4, 150}, {NOTE_C5, 150}, {NOTE_B4, 150}, {NOTE_C5, 150},
    // Fourth phrase: E4 F4 G4 (with a longer final note)
    {NOTE_E4, 150}, {NOTE_F4, 150}, {NOTE_G4, 450}
};
const uint32_t TUNE_LENGTH = sizeof(missionImpossibleTheme) / sizeof(Note);
...
void Buzzer_SetNote(uint16_t frequency) {
    if (frequency == 0) {
        __HAL_TIM_SET_COMPARE(&htim2, TIM_CHANNEL_3, 0); // Stop PWM
    } else {
        uint32_t period = (1000000 / frequency) - 1; // PWM period for 1MHz clock
        __HAL_TIM_SET_AUTORELOAD(&htim2, period);
        __HAL_TIM_SET_COMPARE(&htim2, TIM_CHANNEL_3, period / 2); // 50% duty cycle
        HAL_TIM_PWM_Start(&htim2, TIM_CHANNEL_3);
    }
}
// Starts the non-blocking tune playback
void Buzzer_PlayTune_NonBlocking() {
    note_index = 0;
    Buzzer_SetNote(missionImpossibleTheme[note_index].frequency);
    __HAL_TIM_SET_AUTORELOAD(&htim9, missionImpossibleTheme[note_index].duration);
    HAL_TIM_Base_Start_IT(&htim9);
}

// Timer interrupt callback to play the next note
void HAL_TIM_PeriodElapsedCallback(TIM_HandleTypeDef *htim) {
    if (htim->Instance == TIM9) {
        note_index++;
        if (note_index < TUNE_LENGTH) {
            Buzzer_SetNote(missionImpossibleTheme[note_index].frequency);
            __HAL_TIM_SET_AUTORELOAD(&htim9, missionImpossibleTheme[note_index].duration);
            HAL_TIM_Base_Start_IT(&htim9);
        } else {
            // Tune finished
            Buzzer_SetNote(0); // Stop the buzzer
...
void MX_Buzzer_Init(void) {
    ...
    htim2.Instance = TIM2;
    htim2.Init.Prescaler = 99; // 1MHz timer clock (100MHz / (99+1))
    htim2.Init.Period = 0;
    htim2.Init.ClockDivision = TIM_CLOCKDIVISION_DIV1;
    htim2.Init.CounterMode = TIM_COUNTERMODE_UP;
