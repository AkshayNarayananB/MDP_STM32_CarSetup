...
// Assuming your HCLK is 72 MHz (check your CubeMX Clock Configuration)
#define TIM_CLK_FREQ            72000000

// --- Standard Notes Frequencies (Partial list for the song) ---
typedef enum {
    NOTE_C3  = 131, NOTE_G3 = 196,
    NOTE_C4  = 262, NOTE_D4 = 294, NOTE_E4 = 330, NOTE_G4 = 392, NOTE_A4 = 440,
    NOTE_AS4 = 466, NOTE_C5 = 523,
    NOTE_REST = 0
} Note;


/**
 * @brief Simple millisecond delay function (using HAL_Delay for blocking).
 */
...
/**
 * @brief Sets the Timer PWM to a specific frequency to play a note.
 */
void play_note(uint16_t frequency, uint8_t duty_cycle)
{
    // 1. Stop the channel first
    HAL_TIM_PWM_Stop(&BUZZER_TIMER_HANDLE, BUZZER_CHANNEL);

    if (frequency == 0) {
        return; // Silence, do not start PWM
    }

    uint32_t total_divisor = TIM_CLK_FREQ / frequency;
    uint32_t prescaler = 100;
    uint32_t period;

    // Calculate period based on prescaler
    period = (total_divisor / prescaler) - 1;

    // Adjust prescaler if period is too large (16-bit timer limit)
    if (period > 0xFFFF || period < 100) {
        prescaler = (total_divisor / 0x4000) + 1;
        period = (total_divisor / prescaler) - 1;
    }
    ...
    BUZZER_TIMER_HANDLE.Instance->PSC = prescaler;
    BUZZER_TIMER_HANDLE.Instance->ARR = period;

    // Set the duty cycle (50% for standard tone)
    uint32_t compare = (period * duty_cycle) / 100;
    __HAL_TIM_SET_COMPARE(&BUZZER_TIMER_HANDLE, BUZZER_CHANNEL, compare);
...
/**
 * @brief Plays the iconic Spider-Man theme song (BLOCKING).
 */
void play_spiderman_theme_blocking(void)
{
    const uint32_t QUARTER = 200;
    const uint32_t EIGHTH  = 100;
    const uint32_t DOTTED_EIGHTH = 150;
    const uint32_t SIXTEENTH = 50;
    ...
    const struct NoteSequence song[] = {
        // [SPIDER-MAN, SPIDER-MAN] (Main Riff)
        {NOTE_C4, DOTTED_EIGHTH}, {NOTE_G4, SIXTEENTH}, {NOTE_E4, QUARTER}, {NOTE_REST, EIGHTH},
        {NOTE_G4, DOTTED_EIGHTH}, {NOTE_E4, SIXTEENTH}, {NOTE_D4, QUARTER}, {NOTE_REST, EIGHTH},

        {NOTE_C4, DOTTED_EIGHTH}, {NOTE_G4, SIXTEENTH}, {NOTE_E4, QUARTER}, {NOTE_REST, EIGHTH},
        {NOTE_G4, DOTTED_EIGHTH}, {NOTE_E4, SIXTEENTH}, {NOTE_D4, QUARTER + EIGHTH}, {NOTE_REST, EIGHTH},

        // [Does whatever a spider can...]
        {NOTE_C5, EIGHTH}, {NOTE_AS4, SIXTEENTH}, {NOTE_A4, EIGHTH}, {NOTE_G4, QUARTER + EIGHTH},
        {NOTE_REST, EIGHTH},

        {NOTE_C5, EIGHTH}, {NOTE_AS4, SIXTEENTH}, {NOTE_A4, EIGHTH}, {NOTE_G4, QUARTER + EIGHTH},

        {NOTE_REST, QUARTER} // End silence
    };

    size_t length = sizeof(song) / sizeof(song[0]);

    for (size_t i = 0; i < length; i++)
    {
        play_note(song[i].note, 90); // 50% duty cycle
        delay_ms(song[i].duration);

        // Stop PWM briefly for note separation
        HAL_TIM_PWM_Stop(&BUZZER_TIMER_HANDLE, BUZZER_CHANNEL);
        delay_ms(GAP);
    }
}
...
volatile int32_t encoder_left_counts = 0;
volatile int32_t encoder_right_counts = 0;
float Kp_e = 0.5f; // Proportional gain for encoder PID
float Ki_e = 0.01f; // Integral gain for encoder PID
float Kd_e = 0.05f; // Derivative gain for encoder PID
static float integral_e = 0.0f;
static float last_error_e = 0.0f;

float Kp_h = 30.0f;   // Proportional gain for heading PID
float Ki_h = 4.5f;   // Integral gain for heading PID
float Kd_h = 3.0f;   // Derivative gain for heading PID
...
// --- Corrected Motor_set_pwm function (your version has a bug) ---
...
{
    uint8_t receive_buffer[9 + 1]; // Buffer for 3 chars + NULL terminator

    // 1. Initialize OLED and indicate test status
    OLED_Init();
    OLED_Clear();
    OLED_ShowString(0, 0, (uint8_t*)"AWAITING 3 CHRS...");
    OLED_Refresh_Gram();

    // 2. Blocking Receive Call (Wait for 3 bytes)
    // HAL_MAX_DELAY ensures the MCU waits indefinitely until all 3 bytes are received.
    if (HAL_UART_Receive(&huart3, receive_buffer, 9, HAL_MAX_DELAY) == HAL_OK)
    {
        // 3. Process and Display Result
        receive_buffer[9] = '\0'; // Null-terminate the buffer

        // Clear screen and show the result
        OLED_Clear();
        OLED_ShowString(0, 0, (uint8_t*)"RECEIVED:");

        // Display the 3 characters starting on the next line (or coordinates 0, 16)
        OLED_ShowString(0, 16, receive_buffer);
        OLED_Refresh_Gram();

        // Echo the result back to UART to confirm reception via RPi terminal
        HAL_UART_Transmit(&huart3, (uint8_t*)"Echo: ", 6, HAL_MAX_DELAY);
        HAL_UART_Transmit(&huart3, receive_buffer, 9, HAL_MAX_DELAY);
        HAL_UART_Transmit(&huart3, (uint8_t*)"\r\n", 2, HAL_MAX_DELAY);
    }
    else
    {
        // Display error if the receive failed (e.g., if HAL_UART_Receive returned HAL_ERROR/HAL_TIMEOUT)
        OLED_Clear();
        OLED_ShowString(0, 0, (uint8_t*)"RX FAILED!");
        OLED_Refresh_Gram();
    }
...
// --- Persistent state for my version (prefix: mf_) ---
//static uint32_t mf_last_time = 0;
static int32_t mf_prev_left = 0, mf_prev_right = 0;
static float mf_gz_filt = 0.0f;
...
int main(void)
{
  ...
  HAL_TIM_PWM_Start(&htim1, TIM_CHANNEL_1); //Start buzzer
...
//  Steering_ToUS(10);HAL_Delay(800);
  ...
  //Motor_forward(3000);
 // Motor_set_left_pwm(2500,FORWARD);
 // Motor_set_right_pwm(3000,FORWARD);
  //run_straight_to_distance_cm(150.0,3000);
  //turn_by_angle_degrees(180.0, 3000, 90.0);
  //turn_on_spot_degrees(180.0, 3000);
  //Motor_forward(3000);
  //Motor_forward_2PID(3000);
  ...
  //run_straight_to_distance_cm(100.0, 2000);
  play_spiderman_theme_blocking();
  while (1) {
...
// This callback is executed every time a single byte reception completes.
// Place this function outside of main()
// Place this function outside of main()
void HAL_UART_RxCpltCallback(UART_HandleTypeDef *huart)
{
    // Check if the callback is for your command UART (huart3)
    if (huart->Instance == huart3.Instance)
    {
        // 1. Check for command termination (Newline '\n' or Carriage Return '\r')
        if (ch == '\n' || ch == '\r')
        {
            // Null-terminate the command and signal the main loop
            cmd_buf[cmd_index] = '\0';
            commandReady = true;
            cmd_index = 0; // Reset index for the next command
            OLED_Refresh_Gram(); // Ensure the last character is displayed before clearing
        }
        // 2. Store the character and display it immediately
        else if (cmd_index < CMD_BUF_LEN - 1)
        {
            // Store the received character
            cmd_buf[cmd_index] = ch;

            // Display the single character on the OLED
            // Assuming: X=0 (start column), Y=0 (start row), Size=12 (12x6 font), Mode=1 (Normal)
            // Position X is calculated based on the index * 6 (char width for 12x6 font)
            OLED_ShowChar(cmd_index * 6, 0, ch, 12, 1);
        ...
        {
            // Buffer overflow: Reset the index, clear the display for a fresh start
            cmd_index = 0;
            OLED_Clear();
            OLED_Refresh_Gram();
        }

        // 3. CRITICAL STEP: Re-enable reception for the next single byte
        // This is what allows continuous, multi-byte reception.
        HAL_UART_Receive_IT(&huart3, &ch, 1);
    }
}
