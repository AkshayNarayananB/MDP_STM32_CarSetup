...
int main(void)
{
  ...
  */
  ...
  float gyro_bias_sum = 0.0f;
    const int num_cal_samples = 1000;
...
*/
  ...
  static const char *snap_msg = "SNAP\r\n"; // \r\n added for terminal clarity
  static const uint16_t snap_len = 6;      // Length of "SNAP\r\n"
  ...
  while (1) {
	  ...
	  if (Queue_Dequeue(&next_command))
	  {
		  ...
		  if (cmdQueue.count == 0)
		  {
			  HAL_UART_Transmit(&huart3, (uint8_t*)snap_msg, snap_len, 	HAL_MAX_DELAY);
		  }
	  }
