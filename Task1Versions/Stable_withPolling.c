...
void orbit_from_front(float target_angle, int base_pwm, float steer_angle){
	...
	//Steering_ToUS(0);                         // keep wheels
	//HAL_Delay(1000);
	//uint32_t dist_cm = 999;
	//uint32_t approach_timeout_ms = 7000;      // safety timeout (7s)
	//uint32_t t0 = HAL_GetTick();
	//Motor_set_pwm(base_pwm, base_pwm);
	//dist_cm = HCSR04_Read();
	//while (dist_cm >35){
		//dist_cm = HCSR04_Read();
		 //if (dist_cm <= 35) {                  // reached target standoff
		  //Motor_stop();
		  //HAL_Delay(1000);
		  //break;
		 //}
	//}
	//HAL_Delay(3000);
	//turn_by_angle_degrees(-45.0, 1000, -30.0);
	//turn_by_angle_degrees(45.0, 1000, 20.0);
	//HAL_Delay(1000);
...
*/
  while (1) {
	  ...
	  if (HAL_UART_Receive(&huart3, (uint8_t*)rx_buffer, COMMAND_SIZE, HAL_MAX_DELAY) == HAL_OK) {
		  // Optionally, echo back the command for debugging
	      HAL_UART_Transmit(&huart3, (uint8_t*)rx_buffer, COMMAND_SIZE, HAL_MAX_DELAY);
...
{
    if (huart == &huart3) {
        char tmp[COMMAND_SIZE+1];
        memcpy(tmp, rx_buffer, COMMAND_SIZE);
        tmp[COMMAND_SIZE] = '\0';
        ...
        HAL_UART_Transmit(&huart3, (uint8_t *)tmp, COMMAND_SIZE, HAL_MAX_DELAY);
