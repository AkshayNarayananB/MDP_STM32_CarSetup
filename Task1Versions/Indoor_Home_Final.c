...
void pivot_turn_90_degrees_left()
{
   ...
   // HAL_Delay(500);
    while (fabs(heading) < fabs(PIVOT_TARGET_ANGLE)-46)
    {
        // 1. Brief Motor Pulse
...
void pivot_turn_90_degrees_right()
{
   ...
   // HAL_Delay(500);
    while (fabs(heading) < fabs(PIVOT_TARGET_ANGLE)-40)
    {
        // 1. Brief Motor Pulse
...
void HAL_UART_TxCpltCallback(UART_HandleTypeDef *huart)
{
    if (huart->Instance == USART3)
    {
        // Transmission complete for UART3
        // Optional: toggle an LED or send next message
