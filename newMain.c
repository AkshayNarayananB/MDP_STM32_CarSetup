...
// Write PWM pulse width to servo timer (TIM12 Channel 2)
void Servo_WriteUs(uint16_t us) {
    if(us < 500) us = 500;     // minimum pulse width
    if(us > 2500) us = 2500;   // maximum pulse width
    __HAL_TIM_SET_COMPARE(&htim12, TIM_CHANNEL_2, us);
}

// Set servo angle from -90° to +90°
...
int main(void)
{
  ...
  while (1){
	  ...
	  Motor_forward(2000);
	  Servo_SetAngle(0);   HAL_Delay(800);
	  Servo_SetAngle(90);  HAL_Delay(800);
	  Servo_SetAngle(-90);  HAL_Delay(800);
	  Servo_SetAngle(0); HAL_Delay(800);
	  Servo_SetAngle(90); HAL_Delay(800);

	  /*
	  // 1. Start with servo slightly right and move forward
	  Servo_SetAngle(30);             // small right turn
	  Motor_forward(3000);            // start moving (~36 RPM)
	  HAL_Delay(2000);                // move for 2s

	  // 2. While moving, steer left smoothly
	  for (int angle = 30; angle >= -30; angle -= 5) {
	      Servo_SetAngle(angle);      // gradually sweep right → left
	      HAL_Delay(100);             // smooth transition
	  }
	  HAL_Delay(2000);                // continue moving straight-left for a bit

	  // 3. Now steer back right smoothly
	  for (int angle = -30; angle <= 30; angle += 5) {
	      Servo_SetAngle(angle);      // gradually sweep left → right
	      HAL_Delay(100);
	  }
	  HAL_Delay(2000);

	  // Stop after the S path
	  Motor_stop();
	  Servo_SetAngle(0);              // reset to center
	  /*
